<script lang="ts">
	...
	function handleButtonRef(node: HTMLElement, index: number) {
		// Find the button element within this div
		const buttonElement = node.querySelector('button');
		if (buttonElement) {
			tabButtons[index] = buttonElement;
		}
		return {
			destroy() {
...
>
	{#each modes as mode, i}
		<div use:handleButtonRef={i}>
			<Button
				variant={i === activeIndex ? 'style2' : 'style1'}
				size="small"
